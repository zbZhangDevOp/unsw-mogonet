"""
Utility function files used for training and testing using MOGONET
Mostly inherited from the original MOGONET study
Only function added is move_to_beginning, used for the implementation of cross-vadliation
"""
...
def load_model_dict(folder, model_dict):
    for module in model_dict:
        if os.path.exists(os.path.join(folder, module+".pth")):
            model_dict[module].load_state_dict(torch.load(os.path.join(folder, module + ".pth"), map_location="cpu"))
        else:
...
def move_to_beginning(lst, i, n):
    # Handle wrapping around the list
    length = len(lst)
    if n >= length:
        n = length  # If n is greater than or equal to the list length, it should be the entire list.
    # Calculate the actual range to move
    move_elements = [lst[(i + j) % length] for j in range(n)]
    # Calculate the remaining elements
    remaining_elements = [lst[(i + n + j) % length] for j in range(length - n)]
    # Combine the two parts
    new_lst = move_elements + remaining_elements
    return new_lst